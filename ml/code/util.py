...
def predict(img, MODEL):
    ...
    """
    # Fix image dimensions and run full pipeline
    img = fix_dims(img)
...
def predict_color(img_rgb, model, output_lab=True):
    ...
    # Extract L channel (lightness) and duplicate for model input
    l = image_lab[:, :, [0, 0, 0]]
    # Get model prediction for AB channels
    predicted_ab = model.predict(l[np.newaxis, ...])[0]
    # Apply prediction with scaling factor
...
def upscale_color(original_img, color_lab):
    ...
    # Replace AB channels with predicted ones
    ab = color_lab[:, :, 1:]
    original_lab[:, :, 1:] = ab
