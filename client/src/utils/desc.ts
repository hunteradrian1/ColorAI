...
export const TERMS_AND_CONDITIONS = [
  {
    ...
    text: `Welcome to ColorAI Website! By accessing or using our website or using our API service, you agree to be bound by these terms and conditions.`,
    list: []
  },
  {
  ...
  },
  {
    ...
    list: [
      `Users retain ownership of the images they upload but grant us, ColorAI, a worldwide, irrevocable, non-exclusive, royalty-free license to use, reproduce, adapt, publish, translate and distribute their images in any existing or future media.`,
      ...
      `You are responsible for any claims brought against us, ColorAI, arising out of or related to the images you submit.`
    ]
  },
  {
    heading: "Acceptance of Terms",
...
export const BALANCE_API = `GET http://localhost:4000/api/user/balance/:api_key
...
export const COLORIZE_API = `POST http://localhost:4000/api/image/colorize/:api_key
...
export const IMAGE_API = `GET http://localhost:4000/api/image/get/:image_id
Content-Type: application/json
Body: None
...
export const API_KEY_BLURB = `Below is your local development API access key. This is a simplified
key for local development only. In local mode, you have unlimited balance (999 credits)
and no rate limitations. API keys are generated using a unique device fingerprint
for development purposes.`;

export const API_COLORIZE_BLURB = `To restore color to an image, send a POST request to your local
server endpoint. The image should be sent as a file in the body of the request.
The response will contain a redirect to view the image and a download link.
Images are saved to the local output/ directory.`;

export const API_BALANCE_BLURB = `To check your remaining balance, send a GET request to your local
server endpoint. In development mode, you always have 999 credits (unlimited).
The refresh time is set to 1 hour from creation.`;

export const API_IMAGE_BLURB = `To retrieve a colorized image, send a GET request to your local
server endpoint. The response will contain the colorized image data as a JSON array.
Images are permanently stored in the local output/ directory.`;

export const ABOUT_BLURB = `ColorAI started as a computer vision project at Brown University to
...
export const API_LIMITATION_DESCRIPTION =
  "Local Development Mode: No rate limitations or balance restrictions. Unlimited colorizations for development and testing.";
...
export const PHOTO_PROCESSING_STEPS = [
  ...
  "The 3xWxH tensor is converted to RGB format and saved locally to the output/ directory, and is accessible via the API.",
...
export const LIST_API = `GET http://localhost:4000/api/image/list
Content-Type: application/json
Body: None
...
export const API_DOCUMENTATION_CONTENT = [
  ...
  {
    heading: "List Images",
    text: "Get a list of all colorized image IDs stored locally.",
    code: LIST_API
  }
];
